...
function back(request: Request, suffix = "") {
  return NextResponse.redirect(new URL(`/admin/products${suffix}`, request.url), {
    status: 303,
  });
}
...
export async function POST(request: Request) {
  ...
  if (!url || !key) {
    return back(request, "?error=missing_config");
  }

  if (!slug || !name) {
    return back(request, "?error=missing_required");
  }
  ...
  return response.ok
    ? back(request, "?created=1")
    : back(request, "?error=create_failed");
}
