    ...
    <div className="admin-layout">
      ...
      <main className="admin-main">
        <div className="admin-head">
          <div>
            <h1>Inquiries</h1>
            <p className="muted">View quotation requests, update follow-up status and save internal notes.</p>
          </div>
            ...
            {inquiries.map((i) => {
              ...
              return (
                <tr key={i.id}>
                  ...
                  <td>
                    {i.message}<br />
                    <small>{i.source_page} · {i.created_at ? new Date(i.created_at).toLocaleString() : ""}</small>
                    {rowNotes.length > 0 && (
                      <div className="note-list">
                        {rowNotes.slice(0, 3).map((note) => (
                          <p key={note.id}><b>Note:</b> {note.note}<br /><small>{note.created_at ? new Date(note.created_at).toLocaleString() : ""}</small></p>
                        ))}
                      </div>
                    )}
                  </td>
                  <td>
                    ...
                    <form className="status-form" action="/api/admin/inquiries/update" method="post">
                      <input type="hidden" name="id" value={i.id} />
                      <select name="status" defaultValue={i.status || "new"}>
